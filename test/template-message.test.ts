...
describe("template message", () => {
  it("should parse template message", () => {
    ...
    expect(fn && fn({ name: "CRIMX", fruit: "apple" })).toBe(
      "CRIMX eats apple"
    );
    expect(fn && fn({ name: "CRIMX", fruit: "banana" })).toBe(
      "CRIMX eats banana"
    );
  });
  ...
  it("should fallback to key if args not provided", () => {
    ...
    expect(fn && fn({ fruit: "an apple" })).toBe(
  ...
  it("should support number as value", () => {
    ...
    expect(fn && fn({ zero: 0, one: 1, two: 2 })).toBe("Zero: 0 One: 1 Two: 2");
  });
});
